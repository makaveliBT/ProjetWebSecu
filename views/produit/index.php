	...
	<div class="gtco-section">
		<div class = "gtco-image">
			...
			<form action="panier" method="post" name="$produit[0]['nomModel']">
				<input type="hidden" name="nomProduit" value="<?php echo $produit[0]['nomModel'];?>">
				<input type="hidden" name="nomMarque" value="<?php echo $produit[0]['nomMarque'];?>">
				<input type="number" name="qte" value="1" min="1" max="<?php echo $produit[0]['nombre']; ?>">
				<fieldset>
					<legend>Select a color</legend>
					<?php
						foreach($produit as $product){
							echo '<div>';
								echo '<input type="hidden" name="couleur" value="'. $produit[0]["couleur"].'">';
								echo '<input type="hidden" name="taille" value="'. $produit[0]["pointure"].'">';
								echo '<input type="radio" id="huey"/>';
								echo '<label for="huey">'. $product['couleur'] . " . Taille " . $product['pointure'] . '</label>';
							echo '</div>';
						}
					?>
				</fieldset>
